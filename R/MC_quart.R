...
MC.a.50.q = pblapply(rep("a", 50), execute , cl = clust ) 
stopCluster(clust)

#DGP (b) and Pc = 50
clust = makeCluster(no_cores)
MC.b.50.q = pblapply(rep("b", 50), execute , cl = clust  )
stopCluster(clust)

#DGP (a) and Pc = 100
clust = makeCluster(no_cores)
MC.a.100.q = pblapply(rep("a", 50), execute , Pc = 100, cl = clust  )
stopCluster(clust)

#DGP (b) and Pc = 100
clust = makeCluster(no_cores)
MC.b.100.q = pblapply(rep("b", 50), execute , Pc = 100, cl = clust  )
stopCluster(clust)

setwd("C:/Users/bech/Documents/Dropbox/Project")
save.image(file = "MC_quart.RData")
