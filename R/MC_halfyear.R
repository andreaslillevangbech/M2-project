...
MC.a.50.h = pblapply(rep("a", 50), execute , cl = clust ) 
stopCluster(clust)

#DGP (b) and Pc = 50
clust = makeCluster(no_cores)
MC.b.50.h = pblapply(rep("b", 50), execute , cl = clust  )
stopCluster(clust)

#DGP (a) and Pc = 100
clust = makeCluster(no_cores)
MC.a.100.h = pblapply(rep("a", 50), execute , Pc = 100, cl = clust  )
stopCluster(clust)

#DGP (b) and Pc = 100
clust = makeCluster(no_cores)
MC.b.100.h = pblapply(rep("b", 50), execute , Pc = 100, cl = clust  )
stopCluster(clust)

setwd("C:/Users/bech/Documents/Dropbox/Project")
save.image(file = "MC_half.RData")
