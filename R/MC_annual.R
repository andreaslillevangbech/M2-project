...
execute = function(DGP , Pc=50,  N=100, T=90){
  ...
  df = df %>% mutate(Y = Y + lead(Y, N) + lead(Y,2*N) + 
                       lead(Y,3*N) + lead(Y,4*N) + lead(Y,5*N) +
                       lead(Y,6*N) + lead(Y,7*N) + lead(Y,8*N) +
                       lead(Y,9*N) + lead(Y,10*N) + lead(Y,11*N) )
...
MC.a.50.a = pblapply(rep("a", 50), execute , cl = clust ) 
stopCluster(clust)

#DGP (b) and Pc = 50
clust = makeCluster(no_cores)
MC.b.50.a = pblapply(rep("b", 50), execute , cl = clust  )
stopCluster(clust)

#DGP (a) and Pc = 100
clust = makeCluster(no_cores)
MC.a.100.a = pblapply(rep("a", 50), execute , Pc = 100, cl = clust  )
stopCluster(clust)

#DGP (b) and Pc = 100
clust = makeCluster(no_cores)
MC.b.100.a = pblapply(rep("b", 50), execute , Pc = 100, cl = clust  )
stopCluster(clust)

setwd("C:/Users/bech/Documents/Dropbox/Project")
save.image(file = "MC_annual.RData")
